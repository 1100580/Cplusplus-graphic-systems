...
{
	int x=(int) 0.5+nx+(MAZE_HEIGHT-CHAO_DIMENSAO)/2.0-(1.0-0.5);
	int z=(int) (-0.5)+nz+(MAZE_HEIGHT-CHAO_DIMENSAO)/2.0-(1.0-0.5);
	if(mazedata[z][x]!='*')
		 return GL_FALSE;
	else
		return GL_TRUE;
...
void desenhaCubo()
{
  ...
  desenhaPoligono(vertices[2],vertices[3],vertices[7],vertices[6],normais[2]);
  desenhaPoligono(vertices[3],vertices[0],vertices[4],vertices[7],normais[3]);
  desenhaPoligono(vertices[6],vertices[5],vertices[1],vertices[2],normais[1]);
...
void desenhaBussola(int width, int height)  // largura e altura da janela
{
  ...
  // projeccao ortogonal 2D, com profundidade (Z) entre -1 e 1
	gluOrtho2D(-70, 70, -70*ratio1, 70*ratio1);
  
  // Matriz Modelview
  // Matriz onde s„o realizadas as tranformaÁıes dos modelos desenhados
  glMatrixMode(GL_MODELVIEW);
  glLoadIdentity();
 ...
 glEnable(GL_BLEND);
  ...
  //desenha bussola 2D
	...
	glColor3f(1,0.4,0.4);
	strokeCenterString("N", 0, 20, 0 , 0.3); // string, x ,y ,z ,scale
...
void desenhaModeloDir(objecto_t obj,int width, int height)
{
  ...
  // gluOrtho2D(left,right,bottom,top); 
  // projeccao ortogonal 2D, com profundidade (Z) entre -1 e 1
	gluOrtho2D(-50, 50, -50*ratio1, 50*ratio1);
...
void desenhaLabirinto()
{
	...
	GLfloat meioX,meioZ;
	meioX=-MAZE_WIDTH/2.0+1;
	meioZ=-MAZE_HEIGHT/2.0+1;
	glTranslatef(meioX,0.5,meioZ);
	glColor3f(0,1,0);
...
void setNavigateSubwindowCamera(camera_t *cam, objecto_t obj)
{
	...
	if(!estado.vista[JANELA_NAVIGATE])
  {
	  ...
	  cam->eye.x=obj.pos.x-cos(cam->dir_long)*cos(cam->dir_lat);
	  cam->eye.y=obj.pos.y+0.2+sin(cam->dir_lat);
	  cam->eye.z=obj.pos.z+sin(cam->dir_long)*cos(cam->dir_lat);
    center.x=obj.pos.x;
  ...
  {
	cam->eye.x=obj.pos.x;
    cam->eye.y=obj.pos.y+0.5;
	cam->eye.z=obj.pos.z;
    center.x=obj.pos.x-cos(cam->dir_long)*cos(cam->dir_lat);
    center.y=obj.pos.y+0.2+sin(cam->dir_lat);
  ...
  if(estado.teclas.up){
    // calcula nova posiÁ„o nx,nz
	  ...
	  if(!detectaColisao(nx+OBJECTO_RAIO*cos(modelo.objecto.dir),nz+OBJECTO_RAIO*-sin(modelo.objecto.dir))
		  && !detectaColisao(nx+OBJECTO_RAIO*cos(modelo.objecto.dir-M_PI/4.0),nz+OBJECTO_RAIO*-sin(modelo.objecto.dir-M_PI/4.0))
		  && !detectaColisao(nx+OBJECTO_RAIO*cos(modelo.objecto.dir+M_PI/4.0),nz+OBJECTO_RAIO*-sin(modelo.objecto.dir+M_PI/4.0)))
	  {
