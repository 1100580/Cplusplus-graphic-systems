...
void inicia_jogo()
{
  ...
  modelo.bola.direccao=RAD(rand()%120-60); //direcao entre -60 e 60
  if(modelo.servico==2)
...
*******************************/
void movimentaBola(){
	if(modelo.bola.direccao > 0 && modelo.bola.direccao < M_PI){
		//printf("sobe");
		modelo.bola.y = modelo.bola.y + modelo.bola.velocidade + sin(modelo.bola.direccao);
	}else{
		//printf("desce");
		modelo.bola.y = modelo.bola.y - modelo.bola.velocidade - sin(modelo.bola.direccao);
	}

	if(modelo.bola.direccao>(M_PI/2) && modelo.bola.direccao<(-M_PI/2)){
		modelo.bola.x = modelo.bola.x + modelo.bola.velocidade + cos(modelo.bola.direccao);
	}else{
		modelo.bola.x = modelo.bola.x - modelo.bola.velocidade - cos(modelo.bola.direccao);
	}
}
...
void colisaoParedes(){
	if(modelo.bola.y + modelo.bola.tamanho/2.0 >= ALTURA_CAMPO){
		modelo.bola.direccao = -modelo.bola.direccao;
	}else if(modelo.bola.y - modelo.bola.tamanho/2.0 <= 0) {
		modelo.bola.direccao = -modelo.bola.direccao+2*M_PI;
	}
}

void colisaoRaquete(){
	if(modelo.jogador1.x + LARGURA_RAQUETES/2.0 >= modelo.bola.x - modelo.bola.tamanho/2.0){
		modelo.bola.direccao = -modelo.bola.direccao + M_PI;
	}
	if(modelo.jogador2.x - LARGURA_RAQUETES/2.0 >= modelo.bola.x + modelo.bola.tamanho/2.0){
		modelo.bola.direccao = -modelo.bola.direccao + M_PI;
	}
}
